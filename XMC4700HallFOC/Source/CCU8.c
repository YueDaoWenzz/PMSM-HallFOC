...
void CCU8_Init(void)
{
  SCU_RESET->PRSET0|=(((0x01)<<7)|((0x01)<<8));			//申请对CCU8模块0,1  ccu80进行复位
  SCU_RESET->PRCLR0|=(((0x01)<<7)|((0x01)<<8));//完成复位并退出复位状态
  SCU_CLK->CLKSET|=0x00000010;			  //设置CCU8单元时钟频率
    ...
    //---------------------------------------------------------------------------------------------------------------------
      ...
      //------------------------------------------------------------------------------------------------------------------
				...
				CCU80_CC82->TC|=0x007e0005;  			//CCU8单元计时器计数方式、工作模式，多种外部控制功能的禁止和使能 	p2375
																					//设为 中心对齐，使能shadow transfer，TRAP enabled forCC80的四个脚
																					//TRAP 同步PWM，软件清除TRAP  0x007e0005
